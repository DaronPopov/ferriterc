...
pub fn handle_snapshot(state: &DaemonState) -> io::Result<String> {
    ...
    let response = serde_json::json!({
        ...
        "completion_head": snap.completion_head,
        "completion_tail": snap.completion_tail,
        "completion_overruns": snap.completion_overruns,
    });
    to_json_line(&response)
}
...
pub fn handle_task_submit_v1(state: &DaemonState, args: &[&str]) -> io::Result<String> {
    if args.len() > 6 {
        return Ok("{\"ok\":false,\"error\":\"usage: task-submit-v1 [opcode] [priority] [tenant_id] [flags] [depends_on_task_id] [continuation_opcode]\"}\n".to_string());
    }

    let parse_u32 = |idx: usize, default: u32| -> Result<u32, String> {
        if let Some(raw) = args.get(idx) {
            raw.parse::<u32>()
                .map_err(|_| format!("invalid u32 at arg {} ('{}')", idx + 1, raw))
        } else {
            Ok(default)
        }
    };
    let parse_opt_u32 = |idx: usize| -> Result<Option<u32>, String> {
        if let Some(raw) = args.get(idx) {
            raw.parse::<u32>()
                .map(Some)
                .map_err(|_| format!("invalid u32 at arg {} ('{}')", idx + 1, raw))
    ...
    let opcode = match parse_u32(0, 0) {
        Ok(v) => v,
        Err(e) => return Ok(format!("{{\"ok\":false,\"error\":\"{}\"}}\n", e)),
    };
    let priority = match parse_u32(1, ptx_sys::PTX_PRIORITY_NORMAL) {
        Ok(v) => v,
        Err(e) => return Ok(format!("{{\"ok\":false,\"error\":\"{}\"}}\n", e)),
    };
    let tenant_id = match parse_u32(2, 0) {
        Ok(v) => v,
        Err(e) => return Ok(format!("{{\"ok\":false,\"error\":\"{}\"}}\n", e)),
    };
    let flags = match parse_u32(3, 0) {
        Ok(v) => v,
        Err(e) => return Ok(format!("{{\"ok\":false,\"error\":\"{}\"}}\n", e)),
    };
    let depends_on_task_id = match parse_opt_u32(4) {
        Ok(v) => v,
        Err(e) => return Ok(format!("{{\"ok\":false,\"error\":\"{}\"}}\n", e)),
    };
    let continuation_opcode = match parse_opt_u32(5) {
        Ok(v) => v,
        Err(e) => return Ok(format!("{{\"ok\":false,\"error\":\"{}\"}}\n", e)),
    };
    ...
    desc.tenant_id = tenant_id;
    desc.flags = flags;
    desc.arg_count = 0;
    if let Some(dep) = depends_on_task_id {
        if dep > 0 {
            desc.flags |= ptx_sys::PTX_TASK_FLAG_WAIT_ON_TASK;
            desc.args[ptx_sys::PTX_TASK_META_ARG_DEPENDENCY] = dep as usize as *mut libc::c_void;
        }
    }
    if let Some(op) = continuation_opcode {
        desc.flags |= ptx_sys::PTX_TASK_FLAG_CONTINUATION;
        desc.args[ptx_sys::PTX_TASK_META_ARG_CONTINUATION] = op as usize as *mut libc::c_void;
    }

    let task_id = state.runtime.submit_task_v1(&desc);
    let snap = state.runtime.system_snapshot();
    let response = serde_json::json!({
        "ok": task_id >= 0,
        "task_id": task_id,
        ...
        "continuation_opcode": continuation_opcode,
        "queue_head": snap.queue_head,
        "queue_tail": snap.queue_tail,
        "completion_head": snap.completion_head,
        "completion_tail": snap.completion_tail,
...
pub fn handle_task_poll_v1(state: &DaemonState) -> io::Result<String> {
    let result = state.runtime.poll_completion_v1();
    let snap = state.runtime.system_snapshot();

    let response = match result {
        Some(done) => {
            let isa_context_json = serde_json::Value::Null;

            serde_json::json!({
                "ok": true,
                "empty": false,
                "result": {
                    "abi_version": done.abi_version,
...
fn encode_isa_v0_word(opcode: u8, imm32: u32) -> u64 {
    encode_isa_v0_word_fields(opcode, 0, 0, 0, 0, 0, imm32)
}

fn encode_isa_v0_word_fields(opcode: u8, fmt: u8, rd: u8, rs0: u8, rs1: u8, rs2: u8, imm32: u32) -> u64 {
    (opcode as u64)
        | (((fmt & 0x0f) as u64) << 8)
        | (((rd & 0x1f) as u64) << 12)
        | (((rs0 & 0x1f) as u64) << 17)
...
pub fn handle_task_submit_isa_v0(state: &DaemonState, args: &[&str]) -> io::Result<String> {
    if args.is_empty() || args.len() > 4 {
        return Ok(
            "{\"ok\":false,\"error\":\"usage: task-submit-isa-v0 <tenant_id> [mode] [slice_steps] [priority]\"}\n"
                .to_string(),
        );
    }

    let parse_req_u32 = |idx: usize, name: &str| -> Result<u32, String> {
        let raw = args.get(idx).ok_or_else(|| format!("missing {}", name))?;
        raw.parse::<u32>()
            .map_err(|_| format!("invalid u32 for {} ('{}')", name, raw))
    };
    let parse_opt_u32 = |idx: usize, default: u32| -> Result<u32, String> {
        if let Some(raw) = args.get(idx) {
            raw.parse::<u32>()
                .map_err(|_| format!("invalid u32 at arg {} ('{}')", idx + 1, raw))
        } else {
    ...
    let tenant_id = match parse_req_u32(0, "tenant_id") {
        Ok(v) => v,
        Err(e) => return Ok(format!("{{\"ok\":false,\"error\":\"{}\"}}\n", e)),
    };
    let mode = args.get(1).copied().unwrap_or("halt");
    let slice_steps = match parse_opt_u32(2, 64) {
        Ok(v) if v > 0 => v,
        Ok(_) => return Ok("{\"ok\":false,\"error\":\"slice_steps must be > 0\"}\n".to_string()),
        Err(e) => return Ok(format!("{{\"ok\":false,\"error\":\"{}\"}}\n", e)),
    };
    let priority = match parse_opt_u32(3, ptx_sys::PTX_PRIORITY_NORMAL) {
        Ok(v) => v,
        Err(e) => return Ok(format!("{{\"ok\":false,\"error\":\"{}\"}}\n", e)),
    };

    let code_words: Vec<u64> = match mode {
        "halt" => vec![encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_HALT as u8, 0)],
        "trap" => vec![encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_TRAP as u8, 0xBEEF)],
        "yield" => vec![
            encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_YIELD as u8, 0),
            encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_HALT as u8, 0),
        ],
        "movi" => vec![
            encode_isa_v0_word_fields(ptx_sys::PTX_ISA_V0_OP_MOVI as u8, 0, 1, 0, 0, 0, 7),
            encode_isa_v0_word_fields(ptx_sys::PTX_ISA_V0_OP_ASSERT_EQI as u8, 0, 1, 0, 0, 0, 7),
            encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_HALT as u8, 0),
        ],
        "arith" => vec![
            encode_isa_v0_word_fields(ptx_sys::PTX_ISA_V0_OP_ADD as u8, 0, 1, 0, 0, 0, 0),
            encode_isa_v0_word_fields(ptx_sys::PTX_ISA_V0_OP_ASSERT_EQI as u8, 0, 1, 0, 0, 0, 0),
            encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_HALT as u8, 0),
        ],
        "branch" => vec![
            encode_isa_v0_word_fields(ptx_sys::PTX_ISA_V0_OP_BR_EQ as u8, 0, 0, 0, 0, 0, 2),
            encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_TRAP as u8, 0xBEEF),
            encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_HALT as u8, 0),
        ],
        "jmp" => vec![
            encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_JMP as u8, 2),
            encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_TRAP as u8, 0xBEEF),
            encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_HALT as u8, 0),
        ],
        "pc_oob" => vec![encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_JMP as u8, 10)],
        "mem_ld" => vec![
            encode_isa_v0_word_fields(ptx_sys::PTX_ISA_V0_OP_LD_U32 as u8, 0, 1, 0, 0, 0, 0),
            encode_isa_v0_word_fields(ptx_sys::PTX_ISA_V0_OP_ASSERT_EQI as u8, 0, 1, 0, 0, 0, 0),
            encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_HALT as u8, 0),
        ],
        "mem_oob" => vec![encode_isa_v0_word_fields(
            ptx_sys::PTX_ISA_V0_OP_LD_U32 as u8,
            0,
            1,
            0,
            0,
        ...
        "sys_yield" => vec![
            encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_SYSCALL as u8, ptx_sys::PTX_ISA_V0_SYS_YIELD),
            encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_HALT as u8, 0),
        ],
        "sys_bad" => vec![encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_SYSCALL as u8, 99)],
        _ => {
            return Ok(
                "{\"ok\":false,\"error\":\"mode must be one of: halt, trap, yield, movi, arith, branch, jmp, pc_oob, mem_ld, mem_oob, sys_yield, sys_bad\"}\n"
                    .to_string(),
            )
        }
    };
    ...
    let mut desc = ptx_sys::PTXTaskDescV1::default();
    desc.abi_version = ptx_sys::PTX_TASK_ABI_V1;
    desc.opcode = ptx_sys::PTX_TASK_OPCODE_ISA_RUN;
    desc.priority = priority;
    desc.tenant_id = tenant_id;
    desc.flags = ptx_sys::PTX_TASK_FLAG_COOPERATIVE;
    desc.arg_count = ptx_sys::PTX_MAX_TASK_ARGS;
    desc.args[0] = code_words
        .first()
        .copied()
        .unwrap_or(encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_HALT as u8, 0))
        as usize as *mut libc::c_void;
    desc.args[1] = std::ptr::null_mut(); // null context pointer selects inline ISA mode
    desc.args[2] = slice_steps as usize as *mut libc::c_void;
    desc.args[3] = code_words.len() as usize as *mut libc::c_void;
    desc.args[ptx_sys::PTX_TASK_META_ARG_WORK_REMAINING] = 0usize as *mut libc::c_void; // inline PC
    desc.args[ptx_sys::PTX_TASK_META_ARG_QUANTUM] = 0usize as *mut libc::c_void; // inline steps_total
    desc.args[ptx_sys::PTX_TASK_META_ARG_DEPENDENCY] = code_words
        .get(1)
        .copied()
        .unwrap_or(encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_HALT as u8, 0))
        as usize as *mut libc::c_void; // inline word1
    desc.args[ptx_sys::PTX_TASK_META_ARG_CONTINUATION] = code_words
        .get(2)
        .copied()
        .unwrap_or(encode_isa_v0_word(ptx_sys::PTX_ISA_V0_OP_HALT as u8, 0))
        as usize as *mut libc::c_void; // inline word2
    ...
    let response = serde_json::json!({
        "ok": task_id >= 0,
        "task_id": task_id,
        "opcode": ptx_sys::PTX_TASK_OPCODE_ISA_RUN,
        "tenant_id": tenant_id,
        "priority": priority,
        "slice_steps": slice_steps,
        "mode": mode,
        "code_words": code_words.len() as u32,
    });
    to_json_line(&response)
}

pub fn handle_task_submit_coop(state: &DaemonState, args: &[&str]) -> io::Result<String> {
    if args.is_empty() || args.len() > 4 {
        return Ok("{\"ok\":false,\"error\":\"usage: task-submit-coop <tenant_id> <work_units> [quantum] [priority]\"}\n".to_string());
    }
    let parse_req_u32 = |idx: usize, name: &str| -> Result<u32, String> {
        let raw = args.get(idx).ok_or_else(|| format!("missing {}", name))?;
        raw.parse::<u32>()
    ...
    let work_units = match parse_req_u32(1, "work_units") {
        Ok(v) => v,
        Err(e) => return Ok(format!("{{\"ok\":false,\"error\":\"{}\"}}\n", e)),
    };
    let quantum = match parse_opt_u32(2, 64) {
        Ok(v) => v,
        Err(e) => return Ok(format!("{{\"ok\":false,\"error\":\"{}\"}}\n", e)),
    };
    let priority = match parse_opt_u32(3, ptx_sys::PTX_PRIORITY_NORMAL) {
    ...
    desc.flags = ptx_sys::PTX_TASK_FLAG_COOPERATIVE;
    desc.arg_count = 0;
    desc.args[ptx_sys::PTX_TASK_META_ARG_WORK_REMAINING] = work_units as usize as *mut libc::c_void;
    desc.args[ptx_sys::PTX_TASK_META_ARG_QUANTUM] = quantum as usize as *mut libc::c_void;
...
pub fn handle_task_submit_coop_batch(state: &DaemonState, args: &[&str]) -> io::Result<String> {
    if args.len() < 3 || args.len() > 5 {
        return Ok("{\"ok\":false,\"error\":\"usage: task-submit-coop-batch <tenant_id> <count> <work_units> [quantum] [priority]\"}\n".to_string());
    }
    let parse_req_u32 = |idx: usize, name: &str| -> Result<u32, String> {
        let raw = args.get(idx).ok_or_else(|| format!("missing {}", name))?;
        raw.parse::<u32>()
    ...
    let count = match parse_req_u32(1, "count") {
        Ok(v) if v > 0 => v,
        Ok(_) => return Ok("{\"ok\":false,\"error\":\"count must be > 0\"}\n".to_string()),
        Err(e) => return Ok(format!("{{\"ok\":false,\"error\":\"{}\"}}\n", e)),
    };
    let work_units = match parse_req_u32(2, "work_units") {
        Ok(v) => v,
        Err(e) => return Ok(format!("{{\"ok\":false,\"error\":\"{}\"}}\n", e)),
    };
    let quantum = match parse_opt_u32(3, 64) {
        Ok(v) => v,
        Err(e) => return Ok(format!("{{\"ok\":false,\"error\":\"{}\"}}\n", e)),
    };
    let priority = match parse_opt_u32(4, ptx_sys::PTX_PRIORITY_NORMAL) {
        Ok(v) => v,
        Err(e) => return Ok(format!("{{\"ok\":false,\"error\":\"{}\"}}\n", e)),
    };

    let mut submitted = 0u32;
    let mut first_task_id: i32 = -1;
    let mut last_task_id: i32 = -1;
    for _ in 0..count {
        let mut desc = ptx_sys::PTXTaskDescV1::default();
        desc.abi_version = ptx_sys::PTX_TASK_ABI_V1;
        desc.opcode = 9;
...
pub fn handle_help() -> io::Result<String> {
  ...
  "commands": {
    ...
    "task-submit-v1 [opcode] [priority] [tenant_id] [flags] [depends_on_task_id] [continuation_opcode]": "Submit PTX task descriptor ABI v1 with optional DAG dependency + continuation",
    "task-submit-coop <tenant_id> <work_units> [quantum] [priority]": "Submit cooperative time-sliced workload task (opcode=9)",
    "task-submit-coop-batch <tenant_id> <count> <work_units> [quantum] [priority]": "Submit N cooperative tasks in one command",
    "task-submit-isa-v0 <tenant_id> [mode] [slice_steps] [priority]": "Submit ISA v0 task (modes: halt, trap, yield, movi, arith, branch, jmp, pc_oob, mem_ld, mem_oob, sys_yield, sys_bad)",
    "task-poll-v1": "Poll one PTX task completion ABI v1 entry",
