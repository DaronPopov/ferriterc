...
if [ ! -x "$ENV_SH" ]; then
  ...
  emit_diag "FAIL" "DOC-PREF-0001" "scripts/ptx_env.sh not found or not executable" "restore or chmod +x ferrite-os/scripts/ptx_env.sh"
  exit 1
fi
...
if [ ! -f "${ROOT_DIR}/lib/libptx_os.so" ]; then
  ...
  emit_diag "WARN" "DOC-BUILD-0001" "libptx_os.so not built" "run ./scripts/ptx_build.sh all from ferrite-os"
else
...
if [ ! -f "${ROOT_DIR}/lib/libptx_hook.so" ]; then
  ...
  emit_diag "WARN" "DOC-BUILD-0003" "libptx_hook.so not built" "run ./scripts/ptx_build.sh all from ferrite-os"
else
