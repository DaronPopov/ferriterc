...
LD_LIBRARY_PATH=./lib:$LD_LIBRARY_PATH \
  ...
  --config crates/internal/ptx-daemon/dev-config.toml
```
...
```bash
cd /path/to/ferriterc/ferrite-os
./ferrite-daemon.sh serve --config crates/internal/ptx-daemon/dev-config.toml
...
- `Permission denied` for `/var/run`
  - Use dev config: `--config crates/internal/ptx-daemon/dev-config.toml`
