...
3. Edit `scripts/install.sh` or `scripts/install/lib/*.sh` for CLI/workflow changes (especially `args.sh`, `build.sh`, `cuda.sh`).
...
3. Confirm `LD_LIBRARY_PATH` construction in `scripts/install.sh` and `ferrite-run`.
...
2. Update service installation logic in `scripts/install.sh` if needed.
