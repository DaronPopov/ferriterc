...
use crate::{CpuCtx, GpuCtx, LangError, Result};
...
impl<T> CpuTensor<T> {
    ...
    pub fn with_allocator<F>(ctx: &CpuCtx, shape: Vec<usize>, mut init: F) -> Result<Self>
    where
        T: Copy,
        F: FnMut(usize) -> T,
    {
...
impl<T> GpuTensor<T> {
    ...
    fn from_parts(shape: Vec<usize>, numel: usize, ptr: GpuPtr) -> Self {
        Self {
            shape,
            numel,
            ptr,
...
impl GpuTensor<f32> {
    pub fn relu(&self, ctx: &GpuCtx) -> Result<GpuTensor<f32>> {
        let runtime = ctx.runtime().runtime();
